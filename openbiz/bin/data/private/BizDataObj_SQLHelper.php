...
class BizDataObj_SQLHelper
{
    ...
    public function buildQuerySQL($dataObj)
    {
        ...
        // add table
        $dataSqlObj->addMainTable(BizSystem::getDBTableQuoted($dataObj->m_MainTable));
        // add join table
    ...
    public function buildUpdateSQL($dataObj)
    {
        ...
        {
            $queryString = QueryStringParam::formatQueryString(BizSystem::getDBTableQuoted($col), "=", $val);
            if ($sql!="") $sql .= ", $queryString";
            else $sql .= $queryString;
        }

        $sql = "UPDATE " . BizSystem::getDBTableQuoted($dataObj->m_MainTable) . " SET " . $sql;
    ...
    {   
        ...
        $sql = "UPDATE " . BizSystem::getDBTableQuoted($dataObj->m_MainTable) ." SET ".$setValueStr;
    	if($condition)
        {
    ...
    {
        $sql = "DELETE FROM " . BizSystem::getDBTableQuoted($dataObj->m_MainTable);
    ...
    public function buildInsertSQL($dataObj, $joinValues=null)
    {
        ...
        foreach($sqlFlds as $fldobj)
        {
            ...
            $sql_col .= BizSystem::getDBTableQuoted($col) . ", ";
            //$sql_val .= $_val. ", ";
        ...
        if(is_array($joinValues))
        {
            foreach($joinValues as $joinColumn=>$joinValue)
            {
                ...
                $sql_col .= BizSystem::getDBTableQuoted($joinColumn) . ", ";
                $sql_val .= "'".$joinValue. "', ";
            }
        }
        ...
        $sql = "INSERT INTO  " . BizSystem::getDBTableQuoted($dataObj->m_MainTable) . " (" . $sql_col . ") VALUES (" . $sql_val.")";
        return $sql;
    }
